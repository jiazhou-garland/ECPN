...
if __name__ == '__main__':
    ...
    model = MLEEP(cuda_num=device_num, multi_inputs=True, upsample_type='Transpose-conv', decoder_type='Bidecoder')
    model.cuda()

    #load model------------------------------------------------------------------------------------------------------
    model_ckpt = "MLEEP(multi-inputs).pth"
    model.load_state_dict(torch.load(model_ckpt))

    # prepare data --------------------------------------------------------------------------------------------------
    trainTxtPath = r'train.txt'
    datasets = CustomImageDataset(trainTxtPath)
    ...
    datasets_val = CustomImageDataset(valTxtPath)
    ...
    def train(dataloader, model, loss_, optimizer, IOU, Fscore, epoch, cuda, supervision):
        ...
        for step, (batch_x, batch_y) in enumerate(dataloader): 
            ...
            if supervision:
                for k in range(5):
                    loss += side_weight * loss_(output[k], batch_y, cuda) / batch_size
                loss += fuse_weight * loss_(output[-1], batch_y, cuda) / batch_size
            else:
   ...
   # train and validate in epochs--------------------------------------------------------------------------------------
    ...
    for t in range(epochs):
        print(f"Epoch {t + 1}\n-------------------------------")
        train(train_loader, model, loss3, optimizer, IOU, Fscore, epoch=t, cuda=device_num, supervision=False)
        mean_loss, mean_iou, mean_Fscore = validate(valid_loader, model, loss3, IOU, Fscore, epoch=t, cuda=device_num, supervision=False)
        ...
        torch.cuda.empty_cache()  # 显存将在Nvidia-smi中释放
