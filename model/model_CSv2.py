...
def circ_shift(cen, shift, Connectivity=4):
    ...
    if Connectivity == 4:
        ...
        return B1, B2, B3, B4, B5, B6, B6, B7, B8
    else:
        print("Connectivity should be 4 or 8.")

class ECE_block(nn.Module):
    def __init__(self, in_channel, cuda_num):
        super(ECE_block, self).__init__()
        self.linear_transfer = nn.Conv2d(in_channel, in_channel, 1, 1)
    ...
    def forward(self, x):
        # circle shift
        device = torch.device("cuda:" + str(self.cuda_num))
        B2, B4, B6, B8 = self.circ_shift(x, shift=1, Connectivity=4)

        # linear transformation
        B2_trans = self.linear_transfer(B2)
        B4_trans = self.linear_transfer(B4)
        B6_trans = self.linear_transfer(B6)
        B8_trans = self.linear_transfer(B8)

        Input_trans = self.linear_transfer(x)

        shape = x.shape
        B2_trans_plain = B2_trans.view(-1, shape[1], shape[2] * shape[3])  # b,c,h*w
        B4_trans_plain = B4_trans.view(-1, shape[1], shape[2] * shape[3])  # b,c,h*w
        B6_trans_plain = B6_trans.view(-1, shape[1], shape[2] * shape[3])  # b,c,h*w
        B8_trans_plain = B8_trans.view(-1, shape[1], shape[2] * shape[3])  # b,c,h*w

        Input_g_plain = Input_trans.view(-1, shape[1], shape[2] * shape[3])  # b,c,h*w

        similar_B2 = torch.mul(B2_trans_plain, Input_g_plain)# b,c,h*w
        similar_B2 = torch.sum(similar_B2, dim=1, keepdim=True) # b,1,h*w
        similar_B4 = torch.mul(B4_trans_plain, Input_g_plain)
        similar_B4 = torch.sum(similar_B4, dim=1, keepdim=True)
        similar_B6 = torch.mul(B6_trans_plain, Input_g_plain)
        similar_B6 = torch.sum(similar_B6, dim=1, keepdim=True)
        similar_B8 = torch.mul(B8_trans_plain, Input_g_plain)
        similar_B8 = torch.sum(similar_B8, dim=1, keepdim=True)

        similar = torch.cat((similar_B2,similar_B4,similar_B6,similar_B8),dim=1)# b,4,h*w
        similar = F.softmax(similar, dim=1).to(device)
        B2_weight = torch.index_select(similar, 1, torch.tensor([0]).to(device))
        B4_weight = torch.index_select(similar, 1, torch.tensor([1]).to(device))
        B6_weight = torch.index_select(similar, 1, torch.tensor([2]).to(device))
        B8_weight = torch.index_select(similar, 1, torch.tensor([3]).to(device))

        weighted_B2 = torch.mul(B2_weight, B2_trans_plain.to(device))# b,c,h*w
        weighted_B4 = torch.mul(B4_weight, B4_trans_plain.to(device))
        weighted_B6 = torch.mul(B6_weight, B6_trans_plain.to(device))
        weighted_B8 = torch.mul(B8_weight, B8_trans_plain.to(device))

        weighted_B = weighted_B2 + weighted_B4 + weighted_B6 + weighted_B8 # b,c,h*w
        weighted_B = weighted_B.view(-1, shape[1], shape[2], shape[3]) #b,c,h,w

        output = torch.cat((weighted_B,x), dim=1)#b,2c,h,w
        # print(output.size())
...
class MLEEP(nn.Module):
    ...
    def __init__(self, cuda_num, multi_inputs=False, upsample_type='Transpose-conv', decoder_type=None):
        ...
        self.SE_block = SELayer(channel=14, reduction=7)
        self.c_fusion = SingleConvBlock(14, 1, stride=1, kernel=3, use_bs=True)
        self.decoder_type = decoder_type
        self.cuda_num = cuda_num
        self.ECE_block = ECE_block(in_channel=7,cuda_num=self.cuda_num)
        ...
        if self.multi_inputs:
            ...
            for i in range(3):
                input = (x[:, 0+i, :, :], x[:, 3+i, :, :], x[:, 6+i, :, :], x[:, 9+i, :, :], x[:, 12+i, :, :],
                        x[:, 15+i, :, :], x[:, 18+i, :, :])
                input = torch.stack(input, dim=1)# Bx7xHxW
                input = self.ECE_block(input)# Bx14xHxW
                input = self.SE_block(input) #Bx14xHxW
...
if __name__ == '__main__':
    ...
    inputs = torch.rand(2, 21, 512, 512).to(device)
    model = MLEEP(cuda_num=cuda_num, multi_inputs=True, upsample_type='Transpose-conv', decoder_type='Bidecoder')
    model.to(device)
    print(model)
    o = model(inputs)
    for i in range(6):
